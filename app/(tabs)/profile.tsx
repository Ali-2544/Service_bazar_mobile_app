...
export default function ProfileScreen() {
  ...
  const [userType, setUserType] = useState<'customer' | 'provider' | 'admin'>(
    (user?.role as 'customer' | 'provider' | 'admin') || 'customer'
  );
  ...
  const handleRoleSwitch = () => {
    Alert.alert(
      ...
      [
        { text: 'Customer', onPress: () => setUserType('customer') },
        { text: 'Service Provider', onPress: () => setUserType('provider') },
  ...
  const getUserInfo = () => {
    if (user) {
      return {
        name: user.name,
        email: user.email,
        subtitle: userType === 'admin' ? 'Platform Administrator' : 
                 userType === 'provider' ? 'Service Provider' : 'Customer',
        stats: userType === 'admin' ? 'Super Admin • Full Access' :
               userType === 'provider' ? '4.9 ★ • 124 Reviews • 3 Years Experience' :
               'Member since 2023 • 12 Services Booked',
      };
    }
    switch (userType) {
      case 'provider':
        return {
          ...
          stats: '4.9 ★ • 124 Reviews • 3 Years Experience',
        };
      case 'admin':
        return {
          name: 'Admin User',
      ...
      default:
        return {
          ...
          subtitle: 'Customer',
          stats: 'Member since 2023 • 12 Services Booked',
        };
    }
  };
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.roleSwitcher} onPress={handleRoleSwitch}>
          <Text style={styles.roleSwitcherText}>
            Switch Role: {userType.charAt(0).toUpperCase() + userType.slice(1)}
          </Text>
